...
it('lets a user transfer a star', async () => {
    ...
    let tokenId = 150;
    // 1. create a Star with different tokenId
    await instance.createStar("User 1 Star", tokenId, {from: user1})
...
it('lookUptokenIdToStarInfo test', async () => {
    ...
    let user1 = accounts[1];
    let tokenId = 111;
    let starName = "User 1 Star"
    // 1. create a Star with different tokenId
    await instance.createStar(starName, tokenId, {from: user1})
    // 2. Call your method lookUptokenIdToStarInfo
    let starNameInfo = await instance.lookUptokenIdToStarInfo(tokenId);
    // 3. Verify if you Star name is the same
    assert.equal(starNameInfo, starName);
